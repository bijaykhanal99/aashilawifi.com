...
const saveProduct = () => {
    ...
    if(data.findIndex(product => product.title === title.value) < 0){
        ...
        data = [...data, { title: title.value, qty: qty.value, cp: cp.value, sp: sp.value, mac: mac.value}];
    ...
    if(data != null && data.length > 0){
        ...
        data.map((item, i) => {
            ...
            tbody.innerHTML += `<tr>
                ...
                <td>${item.qty}</td>
                <td>${item.cp}</td>
                <td>Rs. ${parseFloat(item.sp).toFixed(2)}</td>
        ...
        tfoot.innerHTML = `<tr>
                    <td colspan="7"><h1 style="background: #f1f1f1; color: #f00; text-align: center; padding: 40px;">No data create!!!</h1></td>
                </tr>
        `;
    }
},
